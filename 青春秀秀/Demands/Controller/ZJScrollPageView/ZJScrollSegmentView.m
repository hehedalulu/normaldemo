...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        //lulu更改
        CGFloat labelwidth =bounds.size.width+30;
        [self.titleWidths addObject:@(labelwidth)];
...
- (void)setupUI {
    ...
    _scrollView.backgroundColor = [UIColor colorWithRed:55.0/255.0 green:67.0/255.0 blue:81.0/255.0 alpha:1];
...
//            
//        }
        if (self.segmentStyle.XYzuobiao==1.0) {
            
            NSInteger index = 0;
            for (ZJCustomLabel *label in self.titleLabels) {
                
                label.frame = CGRectMake(titleX, titleY, width, titleH);
                index++;
                titleX = index * width+60;
...
//    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.zj_height);
    self.scrollView.frame = CGRectMake(0.0, 0.0, 414, self.zj_height);
    if (self.extraBtn) {
...
- (void)setupScrollLineAndCover {
 ...
 //更改覆盖初始化的地方
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                if (self.segmentStyle.XYzuobiao==1.0) {
                     coverX = (firstLabel.zj_width - coverW) * 0.5+60;
                }
                else{
                    coverX = (firstLabel.zj_width - coverW) * 0.5;
                }
            ...
            self.coverLayer.frame = CGRectMake(coverX, coverY+15, coverW, coverH);
